...
//Main page previewing all movies in array.
app.get('/', (req, res) => {
    /* EDIT CODE HERE ----------------------------------
    render pages/index with data of movies
    -------------------------------------------------- */
    const moviesObject = getmovies();
    const data = {
      movieName: moviesObject
    };
    res.render('pages/index', data);
});


//Routing to individual movie pages via search query.
app.get('/form', (req, res) => {
  const moviesObject = getmovies();
  let searcher = req.query.searchbox;
    for( let i = 0; i < moviesObject.length; i++){
      if (searcher == moviesObject[i].title.toLowerCase()){
        return res.render('pages/movie',{movie:moviesObject[i]});
        }
    }
    return res.send("Movie Id Not Found")
    document.getElementById("myForm").reset();
});


// Routing to individual detailed movie pages
